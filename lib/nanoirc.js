...
function start(options) {
    ...
    completionsPerChannel.onlyChannels = [];
    myChannels.forEach(function (channel) {
        ...
        completionsPerChannel.onlyChannels.push(channel);
    });
    ...
    function completer(line) {
        ...
        return [hits.length ? hits : completionsPerChannel.onlyChannels, line];
    }
    ...
    client = new IRC();
    client.set('nick', myNick);
    if (myPassword) {
        client.set('password', myPassword);
    }
    client.set('address', myServer)
        // .use('logger')
        .use('ctcp')
        .connect(function () {
            myNick = this.nick;
            ...
            client.on('message#', function (from, to, text, msg) {
                printMessage(from, myNick, to, text, msg.timestamp);
            });
            ...
            client.on('action', function (from, to, text) {
                printMessage(from, myNick, to, text);
            });
