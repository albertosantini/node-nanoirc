...
function start({ options }) {
    ...
    function formatter(from, me, to, text = "", timestamp = new Date()) {
        return util.format(
            ...
            timestamp
                .getHours()
                .toString()
                .padStart(2, "0")[myColors.timestamp],
            timestamp
                .getMinutes()
                .toString()
                .padStart(2, "0")[myColors.timestamp],
    ...
    rl.on("line", rawLine => {
        ...
        const isCommandMsg = isCommand && line.slice(1, 4) === "msg";
        ...
        } else if (isCommandMsg) {
            const [, to, message] = line.split(" ");
    ...
    client.addListener("join", (ch, nick) => {
        ...
        if (nick === myNick) {
            printMessage({
                to: nick,
                text: `Joined ${channel[myColors.channel]}`
            });
        } else {
            if (showJoinMessages) {
